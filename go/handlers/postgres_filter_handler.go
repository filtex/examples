...
import (
	...
	"github.com/filtex/filtex-go/builders/postgres"
	"github.com/filtex/filtex-go/builders/postgres/types"
	"github.com/filtex/filtex-go/expressions"
	"net/http"
)

func PostgresFilterHandler(w http.ResponseWriter, r *http.Request) {
	...
	var postgresFilter *types.PostgresExpression
	...
	if expression != nil {
		...
		postgresFilter = &types.PostgresExpression{
			Condition: "1 = 1",
			Args:      []interface{}{},
		}
	}

	results, err := storages.PostgresStorage{}.Query(postgresFilter.Condition, postgresFilter.Args)
	if err != nil {
